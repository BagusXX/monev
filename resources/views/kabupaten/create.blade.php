        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-900 leading-tight">
                    ➕ {{ __('Tambah Kabupaten Baru') }}
                </h2>
                <p class="text-sm text-gray-500 mt-1">Masukkan informasi kabupaten yang ingin ditambahkan</p>
            </div>
            <a href="{{ route('kabupaten.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition font-medium text-sm">
                ← Kembali
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border-2 border-gray-200 hover:shadow-xl transition">
                <div class="p-8 text-gray-900">
                    <form method="POST" action="{{ route('kabupaten.store') }}" class="space-y-6">
                        @csrf
